...
const PromoModal = ({ promo, isOpen, onClose }: PromoModalProps) => {
  ...
  return (
    ...
    >
      <div 
        className="relative max-w-2xl w-full bg-background rounded-3xl shadow-2xl animate-scale-in"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-10 h-10 rounded-full bg-secondary hover:bg-secondary/80 flex items-center justify-center transition-colors group z-10"
          aria-label="Закрыть"
        >
